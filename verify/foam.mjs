// Known Issue (foam) verification: foam bubbles ride the particles. Poses +
// pour frames, with and without the foam pass, plus volume / leaks / errors.
import { loadPlaywright, startServer, launch, outDir, logResult } from './lib.mjs';
import path from 'node:path';

const { chromium } = loadPlaywright();
const dir = outDir('foam');
...
// Same frozen sim state rendered with and without the foam pass.
async function pair(p, name) {
  await freeze(p, true);
  await setPasses(p, { foam: false }); await shot(p, `${name}-nofoam`);
  await setPasses(p, { foam: true }); await shot(p, name);
