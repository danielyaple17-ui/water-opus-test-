// Known Issue (glints) verification: specular glint from a wide-stencil normal.
// Poses + pour, each frozen frame with the old (narrow) and new glint normal.
import { loadPlaywright, startServer, launch, outDir, logResult } from './lib.mjs';
import path from 'node:path';

const { chromium } = loadPlaywright();
const dir = outDir('glint');
...
const setGlint = (p, on) => p.evaluate((on) => { window.__water.renderer.surface.wideGlint = on; }, on);
...
// Same frozen sim state rendered with the narrow (old) and wide (new) glint normal.
async function pair(p, name) {
  await freeze(p, true);
  await setGlint(p, false); await shot(p, `${name}-narrow`);
  await setGlint(p, true); await shot(p, name);
