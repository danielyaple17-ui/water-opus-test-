// M10 verification: final realism pass. Poses (calm, tilt L/R, upside down,
// hard shake, resettled) + a pour frame, rendered with every pass on, plus the
// same frozen frames with the new surface-tangent smoothing off for comparison.
// Volume / leak / console checks as in earlier milestones.
import { loadPlaywright, startServer, launch, outDir, logResult } from './lib.mjs';
import path from 'node:path';

const { chromium } = loadPlaywright();
const dir = outDir('M10');
...
// Same frozen sim state rendered with and without the tangent smoothing.
async function pair(p, name) {
  await freeze(p, true);
  await setPasses(p, { smoothSurface: false }); await shot(p, `${name}-nosmooth`);
  await setPasses(p, { smoothSurface: true }); await shot(p, name);
...
try {
  ...
  await waitSimAbs(pourPage, 0.8);
  await freeze(pourPage, true);
  await shot(pourPage, '01-pour');
