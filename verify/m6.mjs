// M6 verification: foam (impact/spray driven, decaying), rising bubbles and
// surface-driven caustics, over the same pose sequence as M2/M4/M5.
...
try {
  ...
  const snap = () => page.evaluate(() => {
    ...
    return {
      particles: s.count, outside: s.outside, fluidCells: s.fluidCells, fillVolume: +s.fillVolume.toFixed(1),
      bubbles: s.bubbles, meanFoam: +(s.foamSum / s.count).toFixed(4), activity: +s.activity.toFixed(3), stepMs: +s.stepMs.toFixed(2),
  ...
  const phases = [
    ...
    ['06b-just-after-shake', { x: 0, y: G, shake: 0 }, 0.4],
  ...
  const shots = [
    ['09-no-caustics', { caustics: false }],
    ['10-no-foam', { caustics: true, foam: false }],
    ['11-no-bubbles', { foam: true, bubbles: false }],
    ['12-m5-look', { foam: false, caustics: false, bubbles: false }],
  ];
  ...
  await setPasses({ foam: true, caustics: true, bubbles: true });
